...
class InfinityFreeClient:
    ...
    async def get_exam_schedule(self, exam_id: int) -> List[Dict]:
        ...
        return await self._request('exam_schedule', {'exam_id': exam_id})
    ...
    async def get_today_schedule(self) -> List[Dict]:
        ...
        for exam in exams:
            exam_schedule = await self.get_exam_schedule(exam['id'])
            today_schedule = [s for s in exam_schedule if s.get('exam_date') == today]
            schedule.extend(today_schedule)
        
        return schedule
    
    async def get_active_exam_slots(self) -> List[Dict]:
        ...
        for exam in exams:
            schedule = await self.get_exam_schedule(exam['id'])
    ...
    async def get_slot_bookings_by_student(self, student_id: int) -> List[Dict]:
        ...
        return await self._request('slot_bookings', {'student_id': student_id})
    
    # ==================== TOKEN MANAGEMENT ====================
    
    async def validate_token(self, token_no: str) -> Optional[Dict]:
        """Validate exam token"""
        return await self._request('tokens', {'token_no': token_no})
    
    async def can_access_exam(self, token_no: str) -> bool:
    ...
    async def mark_token_used(self, token_no: str, ip_address: str = None) -> Dict:
        """Mark token as used"""
        token = await self.validate_token(token_no)
        if token:
            return await self._request('tokens', method='PUT', data={
                'id': token['id'],
                'status': 'used',
